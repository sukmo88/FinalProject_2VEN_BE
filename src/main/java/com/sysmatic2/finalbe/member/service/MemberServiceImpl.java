...
public class MemberServiceImpl implements MemberService {
    ...
    public void duplicateEmailCheck(String email) {
        ...
        if (memberRepository.findByEmail(email) != null) {
            throw new IllegalArgumentException("Email already in use"); // 사용자 예외(또는 시큐리티 예외)로 변경 필요
        }
    }
    ...
    public void duplicateNicknameCheck(String nickname) {
        ...
        if (memberRepository.findByNickname(nickname) != null) {
            throw new IllegalArgumentException("Nickname already in use"); // 사용자 예외(또는 시큐리티 예외)로 변경 필요
        }
    }
